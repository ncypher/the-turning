...
const machineCap=new THREE.Mesh(
  new THREE.BoxGeometry(5.12,.18,2.62),
  new THREE.MeshStandardMaterial({color:0x292d2f,roughness:.82,metalness:.58})
);
...
function heldPoint(person,key,center,radius=.55,holdMs=18000){
  const now=performance.now();let h=heldDestinations.get(person);
  if(!h||h.key!==key||now>h.until){
    const a=Math.random()*Math.PI*2,r=radius*(.35+Math.random()*.65);
    h={key,until:now+holdMs+Math.random()*9000,point:center.clone().add(new THREE.Vector3(Math.cos(a)*r,0,Math.sin(a)*r))};
...
villagerTarget=function(i,now){
  const v=villagers[Math.max(0,Math.min(villagers.length-1,i-2))]||villagers[i%villagers.length];
  const home=huts[i%huts.length];
  const phase=Math.floor((now+i*2200)/32000)%5;
  if(phase===0)return heldPoint(v,'home-'+i,home.position.clone(),.70,22000);
  if(phase===1)return heldPoint(v,'hearth',activityZones.hearth,.82,20000);
  if(phase===2)return i%2?heldPoint(v,'garden',activityZones.garden,.68,21000):heldPoint(v,'water',activityZones.water,.80,21000);
  if(phase===3)return (state?.techAwake??0)>.25?heldPoint(v,'workshop',activityZones.workshop,.72,22000):heldPoint(v,'homework-'+i,home.position.clone(),.62,22000);
  return i%3===0?heldPoint(v,'machine',activityZones.machine,.70,19000):heldPoint(v,'homeevening-'+i,home.position.clone(),.58,22000);
};
...
animatePerson=function(p,now,dt,target,walkSpeed=1){
  ...
  const u=p.userData;
  const arrived=before<.12||p.position.distanceTo(target)<.10;
  if(arrived){
  ...
  if(u.lookAtPerson&&now<u.lookUntil&&u.lookAtPerson.visible!==false){
    const dx=u.lookAtPerson.position.x-p.position.x,dz=u.lookAtPerson.position.z-p.position.z;
    if(Math.hypot(dx,dz)<3.3){
      ...
      const talk=Math.sin(now*.006+u.phase)*.055;
      u.la.pivot.rotation.x=lerp(u.la.pivot.rotation.x,talk,dt*3);
      u.ra.pivot.rotation.x=lerp(u.ra.pivot.rotation.x,-talk*.45,dt*3);
    }
  }else if(u.lookAtPerson){u.lookAtPerson=null;u.lookUntil=0;}
};
...
function settleMara(){
  if(state&&mara){
    ...
    const dx=mara.position.x-base.x,dz=mara.position.z-base.z;
    if(Math.hypot(dx,dz)<.55){
      const u=mara.userData,s=.18;
      u.leftLeg.pivot.rotation.x=lerp(u.leftLeg.pivot.rotation.x,0,s);
      u.rightLeg.pivot.rotation.x=lerp(u.rightLeg.pivot.rotation.x,0,s);
      u.leftArm.pivot.rotation.x=lerp(u.leftArm.pivot.rotation.x,0,s);
      u.rightArm.pivot.rotation.x=lerp(u.rightArm.pivot.rotation.x,0,s);
      u.torso.rotation.z=lerp(u.torso.rotation.z,0,s);
      mara.position.y=lerp(mara.position.y,0,s);
    }
  }
  requestAnimationFrame(settleMara);
}
requestAnimationFrame(settleMara);
...
function nearestConversationPartner(person){
  const candidates=[mara,eren,...villagers].filter(p=>p&&p!==person&&p.visible!==false);
  let best=null,bestD=Infinity;
  for(const p of candidates){const d=person.position.distanceTo(p.position);if(d<bestD&&d<2.7){best=p;bestD=d;}}
  return best;
}
...
say=function(person,text,duration=4300){
  ...
  if(listener){
    person.userData.lookAtPerson=listener;person.userData.lookUntil=performance.now()+duration+500;
    listener.userData.lookAtPerson=person;listener.userData.lookUntil=performance.now()+Math.min(duration,3200);
  }
...
function holdConversations(){
  ...
  for(const p of [mara,eren,...villagers]){
    ...
    const q=p.userData.lookAtPerson,d=p.position.distanceTo(q.position);
    if(d>1.15&&d<2.8){
      const dir=q.position.clone().sub(p.position);dir.y=0;dir.normalize();
      // only a minute correction; conversation should not yank characters around
      p.position.addScaledVector(dir,.0025);
    }
  }
  requestAnimationFrame(holdConversations);
}
requestAnimationFrame(holdConversations);
